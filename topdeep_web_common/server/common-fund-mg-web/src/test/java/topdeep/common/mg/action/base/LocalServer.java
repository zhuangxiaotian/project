...
public class LocalServer extends AbstractTestNGSpringContextTests {
	...
	@Value("#{test['localServerPort']?:8092}")
	protected int localServerPort = 8090;

	@Value("#{test['localServerContextPath']?:'/mg-web'}")
	protected String localServerContextPath = "/mg-web";
	...
	protected void startServer() throws Exception {
		...
		ResourceHandler resHandle = new ResourceHandler();
		resHandle.setResourceBase(webappPath);
		resHandle.setAliases(true);
		WebAppContext context = new WebAppContext();
		context.setDescriptor(webappPath + "/WEB-INF/web.xml");
		context.setResourceBase(webappPath);
		context.setAliases(true);
		context.setHandler(resHandle);
